...
it('can construct a class', function () {
    expect(new OpenGraphImage(new \Codedor\Seo\Tests\Fixtures\Models\Page(), 'key', 'content'))
...
it('returns empty string if content is null', function () {
    expect(new OpenGraphImage(new \Codedor\Seo\Tests\Fixtures\Models\Page(), 'key', null))
        ->getContent()->toBe('');
});
